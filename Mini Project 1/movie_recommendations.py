...
#Also note that if you have a MxN array A and you index it like A[[0,1,2,0,0,1]], 
#then it will give an array having first row, second row, third row, first row, 
#first row and second row. For example:
#
...
#       [4, 5],
#       [1, 2]])
#This should help you select values corresponding to observations given 
#in y from given likelihood array.

#ESTO ES LO QUE ESTOY HACIENDO AL PASAR y COMO ARGUMENTO DE likelihood

# likelihood[y,:] quiere decir: de likelihood dame los elementos 0, 0 y 1 
# (porque y = [0,0,1]) y de lo que de eso, no lo recortes (porque : entrega todo) 

#Despues sumo en el eje (axis=0), porque tengo que ahcer la sumatoria 
#a traves del eje X
...
def test():
    ...
    num = np.log(likelihood[y, :]).sum(axis = 0) + log_prior
    den = scipy.misc.logsumexp(log_prior*likelihood[y,:].sum(axis=0))
